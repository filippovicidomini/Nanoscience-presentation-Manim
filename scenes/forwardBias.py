...
class PNJunctionForwardBiasWithDopants(Scene):
    def construct(self):
        ...
        # -----------------------------
        # + / - sui contatti (Forward Bias: P al +, N al -)
        # -----------------------------
        minus_label = MathTex("-").set_color(WHITE).scale(0.9)
        plus_label  = MathTex("+").set_color(WHITE).scale(0.9)

        # Posizionali vicino ai punti di contatto superiori (dove sale il filo)
        minus_label.next_to(contact_left + UP*1.5, UP, buff=0.15)   # N side
        plus_label.next_to(contact_right + UP*1.5, UP, buff=0.15)   # P side
